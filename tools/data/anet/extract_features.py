...
if args.model == "swin_b":
    ...
    ckpt_path = "data/pretrained_models/vswin/swin_base_patch244_window877_kinetics600_22k_keysfrom_backbone.pth"#"models/pretrained_models/vswin/swin_base_patch244_window877_kinetics600_22k_keysfrom_backbone.pth"
    FEAT_DIM = 1024
...
def extract_one_video(video_path, dst_path):
    ...
    """
    # print("video_path:",video_path) #/root/yangmin/data/anet/768/training/x3WVWHn3teI
    # assert 1==2
    ...
    for frame_idx in range(0, num_frames, input_length):
        ...
        for i in range(0, len(frames)):
            # print("frames[i].shape:",frames[i].shape)
            img_h, img_w = frames[i].shape[:2]
            frame_resize=(256,-1)
            ...
            frames[i] = mmcv.imresize(frames[i], (new_w, new_h), interpolation="bilinear")
