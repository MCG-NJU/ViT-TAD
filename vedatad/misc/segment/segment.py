...
def segment_overlaps(segments1,
    ...
    if is_aligned:
        ...
        if mode == ['iou','giou']:
            ...
            union = area1
        if mode == 'giou':
            enclosed_lt = torch.min(segments1[..., 0], segments2[..., 0])
            enclosed_rb = torch.max(segments1[..., 1], segments2[..., 1])
    else:
        ...
        if mode == ['iou','giou']:
            ...
            union = area1[:, None]
        if mode == 'giou':
            enclosed_lt = torch.min(segments1[..., :, None, 0],
                                    segments2[..., None, :, 0])
            enclosed_rb = torch.max(segments1[..., :, None, 1],
                                    segments2[..., None, :, 1])
