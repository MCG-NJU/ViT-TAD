...
def bbox_xy_to_cxw(bbox):
    ...
    return torch.cat(bbox_new, dim=-1)
@registry.register_module('match_cost')
class BBoxL1Cost(object):
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        if self.box_format == 'xywh':
            gt_bboxes = bbox_xy_to_cxw(gt_bboxes)
        elif self.box_format == 'xyxy':
            bbox_pred = bbox_cxw_to_xy(bbox_pred)
...
class IoUCost(object):
    ...
    def __call__(self, bboxes, gt_bboxes):
        ...
        overlaps = segment_overlaps( #bbox_overlaps(
